...
const CheckerDetails = () => {
    ...
    useEffect(() => {
        const filtered = checkerData.filter((data) => {
    ...
    }, [searchTerm, filterCriteria, checkerData]);
    ...
    const downloadCSV = () => {
        const csvRows = [
            ['S.No', 'Fleet.No', 'Staff.No', 'Name', 'Route & Service', 'Direction', 'Region', 'Squad', 'Conductor No', 'Passenger Count', 'Ticketless Count', 'Final Collection Amount', 'Stage', 'IR.No', 'Comments', 'Date.of.Submission'],
            ...filteredData.map((data, index) => [
                index + 1,
                data.fleetNumberBasePojo,
                data.staffNumberBasePojo,
                data.staffNameBasePojo || 'NA',
                data.routeAndServicePojo,
                data.directionPojo,
                data.regionPojo,
                data.squadNoPojo,
                data.conductorNoPojo,
                data.passengerCountPojo,
                data.ticketLessTravelPojo,
                data.fineCollectionPojo,
                data.stagePojo,
                data.irNoPojo,
        ...
        link.href = url;
        link.setAttribute("download", "checker_details.csv");
    ...
    return (
        <div className='user-container'>
            ...
            <h3 className='user-heading'>Checker Details</h3>
                ...
                <table className="responsive-table">
                    <thead>
                        <tr>
                            ...
                            <th>Ticketless Count</th>
                            <th>Fine Collection Amount</th>
                            <th>Stage</th>
                            <th>IR No</th>
                            <th>Comments</th>
                        ...
                        {filteredData.length > 0 ? (
                            filteredData.map((data, index) => (
                                <tr key={index}>
                                    ...
                                    <td>{data.routeAndServicePojo || 'NA'}</td>
                                    <td>{data.directionPojo || 'NA'}</td>
                                    <td>{data.regionPojo || 'NA'}</td>
                                    <td>{data.squadNoPojo || 'NA'}</td>
                                    <td>{data.conductorNoPojo || 'NA'}</td>
                                    <td>{data.passengerCountPojo || 'NA'}</td>
                                    <td>{data.ticketLessTravelPojo || 'NA'}</td>
                                    <td>{data.fineCollectionPojo || 'NA'}</td>
                                    <td>{data.stagePojo || 'NA'}</td>
                                    <td>{data.irNoPojo || 'Nill'}</td>
